...
public class KafkaCamelRoute extends RouteBuilder {
  ...
  public KafkaCamelRoute(CamelContext context, Environment environment,
      @Qualifier("dataSource") DataSource dataSource,
      BuildJSONSQLProcessor buildSQLProcessor,
...
//        .retryAttemptedLogLevel(LoggingLevel.ERROR));
    ...
    onException(JsonSyntaxException.class)
        .log(LoggingLevel.ERROR, "JsonSyntaxException in the route ${body}")
    ...
    from("{{fromKafkaRoute}}")
        ...
        .to("{{toKafkaRoute}}")
        .to("{{selectNode}}")
        .log("Read items from DB it ${body}")
        .end();
